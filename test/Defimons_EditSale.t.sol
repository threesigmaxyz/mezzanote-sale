...
contract Defimons_EditSale is DefimonsFixture {
    ...
    function test_editSale_saleNotFoundError() public {
        ...
        // === act ===
        vm.expectRevert(abi.encodeWithSelector(DefimonsStarterMonsters.SaleNotFoundError.selector, MOCK_SALE_ID + 1));
        vm.prank(owner_);
        MOCK_SALE_ID = MOCK_SALE_ID + 1;
        _editMockSale(false);
    }
    ...
    function test_editSale_invalidSaleIntervalError() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                DefimonsStarterMonsters.InvalidSaleIntervalError.selector, MOCK_SALE_START, MOCK_SALE_FINISH
            )
        );
        vm.prank(owner_);
        _editMockSale(false);
    ...
    function test_fuzz_setMaxSaleMint(uint40 newSaleMaxMint_, uint40 numberOfSales_) public {
        ...
        vm.prank(getOwner());
        _editMockSale(false);
        if (loggedNewSaleMaxMint_ != 0) assertEq(defimons.getSale(MOCK_SALE_ID).maxMint, loggedNewSaleMaxMint_);
    }
}
